...
function giant_robot_think(trig_stomp_kill_right, trig_stomp_kill_left, clip_foot_right, clip_foot_left, m_sole, n_robot_id)
{
	...
	if(level flag::get("three_robot_round") && (!isdefined(level.three_robot_round_vo) && level.three_robot_round_vo))
	{
		foreach(player in a_players)
		{
	...
	else if(n_robot_id != 3 && (!isdefined(level.shoot_robot_vo) && level.shoot_robot_vo))
	{
		foreach(player in a_players)
		{
...
function sole_cleanup(m_sole)
{
	...
	util::wait_network_frame();
	m_sole ClearAnim("ai_zm_dlc5_zombie_giant_robot_hatch_close", 0);
	util::wait_network_frame();
...
function giant_robot_head_player_eject_thread(m_linkpoint, str_tube, b_timeout)
{
	...
	self enableWeapons();
	if(isdefined(var_1bcd223d) && var_1bcd223d != level.weaponNone)
	{
		self SwitchToWeaponImmediate(var_1bcd223d);
	}
	...
	/#
		debug_level = GetDvarInt("Dev Block strings are not supported");
...
function watch_for_force_giant_robot()
{
	/#
		while(1)
		{
			...
			{
				SetDvar("Dev Block strings are not supported", "Dev Block strings are not supported");
				if(isdefined(level.devgui_force_giant_robot_foot) && level.devgui_force_giant_robot_foot == "Dev Block strings are not supported")
				{
				...
				{
					level.devgui_force_giant_robot_foot = "Dev Block strings are not supported";
					IPrintLnBold("Dev Block strings are not supported");
				}
			}
			if(GetDvarString("Dev Block strings are not supported") == "Dev Block strings are not supported")
