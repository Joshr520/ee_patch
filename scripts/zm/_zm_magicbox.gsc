...
function init_starting_chest_location( start_chest_name )
{
	...
	else
	{
		for( i = 0; i < level.chests.size; i++ )
		{
			...
			{
				if(IsSubStr(level.chests[i].script_noteworthy,  "basement_chest" ))
				{
					level.chest_index = i;
...
function wonder_weapon_status()
{
	foreach (player in GetPlayers())
	{
		if (player zm_weapons::has_weapon_or_upgrade(GetWeapon("raygun_mark3"))) return 1;
	}
	return 0;
}
...
function treasure_chest_ChooseWeightedRandomWeapon( player )
{
	if(level.script == "zm_stalingrad")
	{
		if(!wonder_weapon_status()) return GetWeapon("raygun_mark3");
		else if(!player zm_weapons::has_weapon_or_upgrade(GetWeapon("lmg_cqb"))) return GetWeapon("lmg_cqb");
		else if(!player zm_weapons::has_weapon_or_upgrade(GetWeapon("cymbal_monkey"))) return GetWeapon("cymbal_monkey");
		else if(!player zm_weapons::has_weapon_or_upgrade(GetWeapon("sniper_fastsemi"))) return GetWeapon("sniper_fastsemi");
	}
